...
- **Read** (default) — one page at a time via the shared image pipeline. `n` / `p` step pages,
  the status line shows `page X/Y`. Zoom / pan via the standard keys
  ([Zoom & pan](../viewer/zoom-pan.md)). Per-page cache keyed by
  `(cols, rows, style, image-mode, background, fit)`; resizing or cycling render settings
  re-renders only the visible page.
