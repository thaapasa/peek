...
//! `render_section(lines, &self, theme)` via [`impl_info_extras!`]. The
//! impls live here (not in `info/`) because the structs are owned by the
//! `types/` layer: the trait is upstream, the type is local, so the impl
//! is orphan-legal in this crate — and stays legal once `types/` is split
//! into its own crate below `info`. Both arguments name the real
//! `crate::types::…` paths (no detour through `info` re-exports), so this
//! file is the only `types → info` edge: the `InfoExtras` trait itself.
//! Adding a file type means adding one row here.
