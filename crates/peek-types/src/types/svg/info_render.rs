...
pub fn render_section(lines: &mut Vec<String>, stats: &SvgStats, theme: &PeekTheme) {
    render_info(lines, &SvgView::from(stats), theme);
}

/// Typed `--info --json` view of the SVG section, nested under `"svg"`.
pub fn json_section(stats: &SvgStats) -> (&'static str, serde_json::Value) {
    (
        "svg",
        serde_json::to_value(SvgView::from(stats)).expect("svg info view serializes"),
    )
}
