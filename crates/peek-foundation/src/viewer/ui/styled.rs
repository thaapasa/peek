//! The SGR-aware string family: every helper that walks a styled string
//! while honouring its escape sequences — tab expansion, visible-width
//! counting, wrapping (char- and word-boundary), horizontal slicing, and
//! truncation. Shared invariants: escapes are zero width, wide chars
//! never split, combining marks attach to their preceding cell, and the
//! active fg/bg style survives a cut via [`ActiveStyle`] re-emission.
...
pub fn wrap_styled_words(s: &str, width: usize) -> Vec<String> {
    if width == 0 {
        return vec![String::new()];
    }
    let s = expand_tabs(s);
...
pub fn count_wrap_segments(s: &str, width: usize) -> usize {
    if width == 0 {
        return 1;
    }
    let s = expand_tabs(s);
...
pub fn take_cols(s: &str, max_cols: usize) -> String {
    let s = expand_tabs(s);
...
pub fn slice_styled_h(s: &str, start_col: usize, max_cols: usize) -> String {
    if max_cols == 0 {
        return String::new();
    }
    let s = expand_tabs(s);
...
///
/// Not `slice_styled_h(s, 0, max_width)`: this is the byte-faithful crop
/// for the final terminal write. Every escape up to the cut passes
/// through raw — including non-color attributes (bold / italic /
/// underline) that [`ActiveStyle`] doesn't track and `slice_styled_h`
/// would therefore drop when it re-synthesises the head style. No
/// trailing reset is appended either: `ScreenBuffer::draw` brackets
/// every row with its own style-mode-aware reset bytes, so adding one
/// here would only pad the hot draw path.
