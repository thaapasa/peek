...
use super::content_window::{WindowRenderer, usable_width};
...
///
/// The window-render path (prepare + visual-row emission) lives in
/// [`WindowRenderer`]; the pipe path in
/// [`content_pipe`](super::content_pipe). This file keeps state,
/// keystrokes, search wiring, and the [`Mode`] impl.
...
impl ContentMode {
    ...
    fn usable_width(&self, total: usize) -> usize {
        usable_width(self.cached_cols, &self.gutter, total)
    }
    ...
    /// Bundle the field borrows the window-render path needs into a
    /// [`WindowRenderer`]. Leaves `self.wrap` shared and the rest of
    /// `self` untouched, so the post-render `clamp_top` (which needs
    /// `&mut self.wrap`) runs after the renderer drops.
    fn window_renderer(&mut self) -> WindowRenderer<'_> {
        WindowRenderer {
            rendering: &mut self.rendering,
            line_source: &self.line_source,
            highlighter: &mut self.highlighter,
            syntax_token: self.syntax_token.as_deref(),
...
impl Mode for ContentMode {
    ...
    fn render_window(&mut self, ctx: &RenderCtx, _scroll: usize, rows: usize) -> Result<Window> {
        ...
        // size-cap refusal; the renderer re-checks `is_pretty_ready`
        // against the post-flip state.
        let mut renderer = self.window_renderer();
        let (styled, top_logical, total) = renderer.prepare(ctx, rows)?;
        let lines = renderer.emit(ctx, rows, &styled, top_logical, total);
        let window = Window { lines, total };
    ...
    fn set_search(&mut self, query: Option<&str>) -> SearchTarget {
        ...
        if let Some(stop) = search.stop() {
            ...
            self.pending_warnings
                .push(search::truncated_scan_warning(stop, ""));
        }
