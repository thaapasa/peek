...
pub use about::AboutMode;
pub use content::{ContentMode, ContentModeConfig};
...
impl<'a> RenderCtx<'a> {
    ...
    pub fn capped_for_image_pipe(&self) -> RenderCtx<'a> {
        RenderCtx {
...
pub fn slice_window(lines: &[String], scroll: usize, rows: usize) -> Vec<String> {
...
pub fn step_search(search: &mut Option<SearchState>, delta: isize) -> Handled {
...
pub trait Mode {
    ...
    /// expected to scroll the viewport. `None` clears any active search
    /// (the session maps an empty query to `None` before it reaches
    /// here). Default no-op (`Owned`) for modes without search.
    fn set_search(
        &mut self,
        _query: Option<&crate::viewer::search::SearchQuery>,
    ) -> crate::viewer::search::SearchTarget {
