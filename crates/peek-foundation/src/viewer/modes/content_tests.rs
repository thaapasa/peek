...
/// Synthetic multi-line Rust source for the highlight-window test. Built
/// in-memory (no on-disk fixture) so the test is self-contained in this
/// crate: a leading multi-line block comment exercises the highlighter's
/// cross-line state, then enough short functions to clear the >50-line
/// bar with every line under the 80-col make_ctx width.
fn rust_fixture() -> InputSource {
    let mut s = String::from(
        "/* Synthetic Rust source.\n   Spans several lines so the block-comment\n   highlight state crosses window boundaries. */\n",
    );
    for i in 0..60 {
        s.push_str(&format!("fn function_number_{i}(value: u32) -> u32 {{\n"));
        s.push_str(&format!("    let scaled = value.wrapping_mul({i});\n"));
        s.push_str("    scaled.wrapping_add(1)\n");
        s.push_str("}\n");
    }
    InputSource::stdin(Bytes::from(s.into_bytes()))
}
...
/// Uses a synthetic Rust source so the test goes through the full
