...
/// Generate the canonical `render_section` / `json_section` free
/// functions for a stats struct that projects into a view via
/// `From<&$stats>`. The view drives both outputs — themed print through
/// [`render_info`], JSON through `serde_json::to_value` nested under
/// `$key`. Emits the pair into the calling module so the view struct
/// stays module-private; [`impl_info_extras!`]'s three-argument form
/// then wires them. Types whose view isn't a plain `From` projection
/// (newtype-ref wrappers, hand-built `InfoView`, bespoke `json_section`)
/// spell the two functions out by hand instead.
#[macro_export]
macro_rules! info_section {
    ($stats:ty, $view:ty, $key:literal) => {
        /// Themed terminal info section (generated by `info_section!`).
        pub fn render_section(
            lines: &mut ::std::vec::Vec<::std::string::String>,
            stats: &$stats,
            theme: &$crate::theme::PeekTheme,
        ) {
            $crate::info::render_info(lines, &<$view>::from(stats), theme);
        }

        /// Typed `--info --json` view, nested under `$key` (generated by
        /// `info_section!`).
        pub fn json_section(stats: &$stats) -> (&'static str, ::serde_json::Value) {
            (
                $key,
                ::serde_json::to_value(<$view>::from(stats))
                    .expect(::std::concat!($key, " info view serializes")),
            )
        }
    };
