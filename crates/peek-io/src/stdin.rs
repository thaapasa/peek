...
/// Read all of piped stdin into an in-memory [`InputSource`], then reopen
/// fd 0 from the controlling terminal so the interactive crossterm event
/// loop can still read keystrokes.
///
...
pub fn read_stdin() -> Result<InputSource> {
    ...
    Ok(InputSource::stdin(Bytes::from(buf)))
}
