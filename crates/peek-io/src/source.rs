...
impl InputSource {
    ...
    ///
    /// Streams via [`ByteStream`](crate::stream::ByteStream)'s `BufRead`
    ...
    pub fn byte_to_line(&self, byte: u64) -> Option<usize> {
        ...
        let mut stream = crate::stream::ByteStream::range(bs, 0, limit);
        let mut count = 0usize;
        loop {
    ...
    pub fn open_line_source(&self) -> Result<crate::LineSource> {
        crate::LineSource::open(self)
    }
    ...
    pub fn open_stream(&self) -> Result<crate::ByteStream> {
        Ok(crate::ByteStream::open(self.open_byte_source()?))
    }
...
/// [`ByteStream`](crate::stream::ByteStream) for sequential walks.
// `len` here is the source's byte length, not a collection size — an
// `is_empty` companion would be meaningless for a random-access reader.
